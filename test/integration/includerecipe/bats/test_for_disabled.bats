...
@test "test rpmforge is disabled" {
    cat /etc/yum.repos.d/rpmforge.repo  | grep 'enabled=0'
}

@test "test rpmforge-extras is disabled" {
    cat /etc/yum.repos.d/rpmforge-extras.repo  | grep 'enabled=0'
}

@test "test rpmforge-testing is disabled" {
    cat /etc/yum.repos.d/rpmforge-testing.repo  | grep 'enabled=0'
}
